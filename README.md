...
> **快捷上手**：仓库根目录提供了 `INIT_CONFIG.example.json`，已收录 35+ 个常用 maccms V10 影视资源站与 6 个 IPTV 直播源（含 GitHub 自动维护的源）。可直接复制其中的内容到环境变量 `INIT_CONFIG`，或粘贴到管理后台的配置文件设置中。代码内 `src/lib/default-sources.ts` 会在未配置任何源时自动注入相同的默认值。
